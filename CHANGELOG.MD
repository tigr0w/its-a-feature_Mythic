...
- Fixed an issue where building payloads while c2 profiles were syncing would double issue tasks to c2 profile and hang
- Fixed an issue here cached p2p paths weren't getting flushed properly

## [3.4.21] - 2026-01-14 

### Changed

- Fixed an issue where downloaded files weren't having their sizes reflected after completion in the file browser

## [3.4.20] - 2026-01-08

### Changed

- Updated IterateAndAct to treat a crypto value of "" the same as "none"

## [3.4.19] - 2025-12-31

### Changed

- Updated processing of skipped steps to indicate skip instead of most recent step that was updated

## [3.4.18] - 2025-12-23

### Changed

- Updated how processing happens for the `update_deleted` with file browser data
  - data is now processed as part of two channels to ensure ordering and supporting multi-chunk file browser data
  
## [3.4.17] - 2025-12-23

### Changed

- Fixed an issue where uploaded files were getting treated as folders in the file browsers
- Fixed an issue where uploaded files would get the wrong timestamp in the file browser
- Updated go modules
...
- Fixed an issue where supplying a cron job in eventing as something other than string would crash due to type conversion

## [3.4.15] - 2025-12-22

### Changed

- Fixed an issue with translation containers sometimes not getting associated with their payload types on first sync

## [3.4.14] - 2025-12-18

### Changed

- Added database triggers for `filemeta`, `credential`, and `keylog` to update timestamp when other fields change

## [3.4.13] - 2025-12-15

### Changed

- Updated MythicRPCTaskCreate to require OperatorID, TaskID, or EventStepInstanceID
  - This is an update to make sure the new task is associated with the right user and not the callback creator
- Updated the default tasking to Split Tasking instead of the collapsable accordian tasking

## [3.4.12] - 2025-12-11

### Changed

- Added more context to the Proxy start event log message
- Added a new event log message when a Proxy stops
- Updated the CustomBrowserExportFunctionMessage to also take in a callback group
- Updated the SendMythicRPCFileCreate call to also allow creation by OperationID
...
- Added the following new options for BuildParameter's HideConditionOperator
  - LessThan, GreaterThan, LessThanOrEqual, GreaterThanOrEqual, StartsWith, EndsWith, Contains, NotContains
- [UI Updates](/MythicReactUI/CHANGELOG.MD#0383---2025-12-10)
- [CLI Updates](/Mythic_CLI/Changelog.md#0321---2025-12-10)
...
- Fixed an issue with eventing where payload data wasn't getting passed to new callback triggers in some situations

## [3.4.8] - 2025-10-29

### Changed

- Added support for a new MythicRPCCallbackEdgeRemove RPC call to allow developers to selectively remove edges
- Added support for a new MythicRPCHandleAgentJson RPC call to allow developers to submit standard Mythic JSON messages here for processing after using a translation container instead of hitting a bunch of various other RPC calls to create their needed objects
- Updating MythicRPCCallbackEdgeSearch to also return self edges - i.e. an egress' edge to Mythic
  - This is a change where before it would only return P2P edges in the graph
- Updated MythicRPCTaskCreate to take in either AgentCallbackID or CallbackID
- Added a few more spots in agent message processing to emit error information to the operation event log
  
## [3.4.7] - 2025-10-28

### Changed

- Added "ui_position" fields to BuildParameters and C2 Profile Parameters for display ordering when building payloads
- Added "dynamic_query_function" option to BuildParameters to allow dynamic execution and choice creation during build process
- Added "copy_of_file_id" to FileMeta to better tracking when Mythic creates copies of files 
  - When uploading fileA to callback1, Mythic tracks this association so it can record things like upload location
  - If fileA (by same UUID) is uploaded again (callback1 with a different task or callback2), Mythic creates a copy of the file meta and tracks it
    - This allows Mythic to track the new upload location separate from the first upload
    - Both of these files are backed by the same file on disk, so deleting one deletes all copies
    - This is now better represented in the UI on the file search page
